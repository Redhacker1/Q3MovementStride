...
namespace MyFPSTest.Player
{
    public class PlayerInput : SyncScript
    {
        ...
        public static readonly EventKey<bool> QueueJump_Event = new EventKey<bool>();
        ...
        public override void Update()
        {
            ...
            //  For this reason we map the 2D user input to a 3D movement using the current camera
            {
                ...
                if (KeysDown.Any(key => Input.IsKeyDown(key)))
                    moveDirection += -Vector2.UnitY;
                if (KeysDown.Any(key => Input.IsKeyDown(Keys.Space)))
                {
                    QueueJump = true;
                }
                ...
                var worldSpeed = Utils.LogicDirectionToWorldDirection(moveDirection, Camera, Vector3.UnitY); ; // If we don't have the correct camera attached we can send the directions anyway, but they probably won't match

                MoveDirectionEventKey.Broadcast(worldSpeed);
                QueueJump_Event.Broadcast(QueueJump);
            }
